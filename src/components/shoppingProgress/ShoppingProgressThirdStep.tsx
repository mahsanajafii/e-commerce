  ...
  return (
    <div className="w-full h-full flex justify-center items-center">
      <div className="  w-full flex flex-col justify-center items-center gap-20">
        ...
        <div className="w-5/6  gap-16 flex flex-col ">
          ...
          <div className=" h-[30rem] gap-8 w-full flex flex-col justify-between items-center">
            <p className="w-full h-12  text-[2.4rem] leading-10  font-medium">خلاصه خرید</p>
            <div className="bg-base-side w-full  flex flex-row justify-between items-center p-12 rounded-xl">
              {infoItems.map(({ label, value }) => (
                <div key={label} className="flex flex-col h-28 gap-6 justify-center items-start w-1/4">
                  <p className="h-12  text-[2.4rem] leading-10  font-medium">{label}</p>
                  <span className="text-text-secondary text-[1.6rem] font-bold">
                    {label.split(" ")[0]} : <span className="text-text-primary text-[1.6rem] font-normal">{value}</span>
                  </span>
                </div>
              ))}
      ...
      {summaryItems.map(({ label, value }) => (
        <div className="w-full flex flex-row justify-between items-start  text-[1.6rem] leading-10 h-10" key={label}>
