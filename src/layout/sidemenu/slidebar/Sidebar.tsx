    ...
    >
      {countOfProduct !== 0 && i === 2 ? (
        <div className=" relative inline-block">
          <span className="top-[-.3rem] right-[-.1rem]  text-white border rounded-[50%]  px-1 text-xs absolute bg-red-600">
            {countOfProduct}
          </span>
          <span>{icon}</span>
        </div>
      ...
      <span
        className={`overflow-hidden transition-width h-5 font-normal leading-5 text-[1.6rem]  duration-700 ease-in-out ${
          expanded ? "w-fit" : "w-0"
        }`}
      >
        {name}
